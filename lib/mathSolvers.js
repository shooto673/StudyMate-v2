...
//   graphData,   // attached via buildGraphFromSpec — null for arithmetic-only
// }
import { buildGraphFromSpec } from './buildGraphFromSpec.js'
...
function _generateSimilarityRatioLength(rng = Math.random) {
...
function _generateExteriorAngle(rng = Math.random) {
  let attempt = 0
  while (attempt++ < 100) {
...
function _generateTriangleAngleSum(rng = Math.random) {
  let attempt = 0
  while (attempt++ < 100) {
...
function _generateRatioLength(rng = Math.random) {
...
function _generateRatioSimplify(rng = Math.random) {
...
// ─────────────────────────────────────────────────────────────
// 6. thales_theorem: 円Oの直径ABと円周上の点C → ∠ACB=90°
//    given: angleBAC → asked: angleABC = 90 - angleBAC
//    (タレスの定理を使った角度問題)
// ─────────────────────────────────────────────────────────────
export function solveThalesTheorem({ angleBAC }) {
  if (!Number.isFinite(angleBAC) || angleBAC <= 10 || angleBAC >= 80) {
    throw new Error('thales_theorem: invalid angle')
  }
  const answer = 90 - angleBAC
  const trace = [
    `ABは円Oの直径なので、タレスの定理より∠ACB=90°`,
    `△ABCの内角の和=180°より、∠ABC=180°-90°-${angleBAC}°=${answer}`,
  ]
  return { answer, trace }
}

function _generateThalesTheorem(rng = Math.random) {
  let attempt = 0
  while (attempt++ < 50) {
    const angleBAC = 20 + Math.floor(rng() * 50) // 20..69
    if (angleBAC === 45) continue
    let result
    try { result = solveThalesTheorem({ angleBAC }) } catch { continue }
    const { answer, trace } = result
    const questionText = `円Oの直径をABとする。円周上に点Cをとるとき、∠BAC=${angleBAC}°である。このとき、∠ABCの大きさを求めなさい。`
    const explanation = trace.join('。') + '°。'
    const distractors = [
      angleBAC,
      180 - answer,
    ...
    return {
      spec: {
        problemType: 'thales_theorem',
        given: { angleBAC },
        computed: { answer, angleACB: 90 },
        solverTrace: trace,
      },
      question: questionText,
      choices,
      ...
      correctAnswer,
      explanation,
      hint: '直径に対する円周角は90°（タレスの定理）だよ！',
    }
  }
  throw new Error('thales_theorem: failed to generate')
}

// ─────────────────────────────────────────────────────────────
// 7. cyclic_quadrilateral: 円に内接する四角形の対角の和=180°
// ─────────────────────────────────────────────────────────────
export function solveCyclicQuadrilateral({ angleABC }) {
  if (!Number.isFinite(angleABC) || angleABC <= 30 || angleABC >= 150) {
    throw new Error('cyclic_quadrilateral: invalid angle')
  }
  const answer = 180 - angleABC
  const trace = [
    `円に内接する四角形の対角の和は180°`,
    `∠ADC = 180° - ∠ABC = 180° - ${angleABC}° = ${answer}`,
  ]
  return { answer, trace }
}

function _generateCyclicQuadrilateral(rng = Math.random) {
  let attempt = 0
  while (attempt++ < 50) {
    const angleABC = 40 + Math.floor(rng() * 100) // 40..139
    if (angleABC === 90) continue // rectangle ambiguity
    let result
    try { result = solveCyclicQuadrilateral({ angleABC }) } catch { continue }
    const { answer, trace } = result
    const questionText = `円に内接する四角形ABCDがあり、∠ABC=${angleABC}°である。∠ADCの大きさを求めなさい。`
    const explanation = trace.join('。') + '°。'
    const distractors = [
      angleABC,
      answer + 10,
      answer - 10,
      Math.abs(angleABC - 90),
    ].filter(v => Number.isInteger(v) && v > 0 && v < 180 && v !== answer)
    const { choices, correctIndex, correctAnswer } =
      buildChoices(answer, distractors, v => `${v}°`, rng)
    return {
      spec: {
        ...
        given: { angleABC },
        computed: { answer, angleADC: answer },
        solverTrace: trace,
      },
      question: questionText,
      choices,
      ...
      correctAnswer,
      explanation,
      hint: '円に内接する四角形の対角の和は180°だよ！',
    }
  }
  throw new Error('cyclic_quadrilateral: failed to generate')
}

// ─────────────────────────────────────────────────────────────
// Public exports + registry.
// Every generator is wrapped so the returned question always carries
// `graphData` derived from the canonical spec — the single source of truth.
// This satisfies the validator's `missing_required_graph` rule without
// duplicating figure-building code per generator.
// ─────────────────────────────────────────────────────────────
function withGraph(fn) {
  return (rng) => {
    const q = fn(rng)
    return { ...q, graphData: buildGraphFromSpec(q.spec) }
  }
}

export const generateSimilarityRatioLength = withGraph(_generateSimilarityRatioLength)
export const generateExteriorAngle         = withGraph(_generateExteriorAngle)
export const generateTriangleAngleSum      = withGraph(_generateTriangleAngleSum)
export const generateRatioLength           = withGraph(_generateRatioLength)
export const generateRatioSimplify         = withGraph(_generateRatioSimplify)
export const generateThalesTheorem         = withGraph(_generateThalesTheorem)
export const generateCyclicQuadrilateral   = withGraph(_generateCyclicQuadrilateral)
...
export const GENERATORS = {
  ...
  thales_theorem: generateThalesTheorem,
  cyclic_quadrilateral: generateCyclicQuadrilateral,
}
